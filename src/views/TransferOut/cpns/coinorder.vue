    ...
    <van-list class="list" v-model="loading" :finished="finished" :finished-text="$t('No more')" @load="loadData">
      <div class="recorditem" v-for="(item,index) in list" :key="index">
        <div class="line">
          ...
          <span>{{$t('oneDay')}} +{{item.interest_rate | rateFormat}} [{{$t('set')}}] {{item.hold_cycle}} {{$t('days')}}</span>
        </div>

        <div class="line">
          <span>{{$t('depositAmount')}}:</span>
          <span>{{item.exp_amount | moneyFormat}}</span>
        </div>

        <div class="line">
          <span>{{$t('expectedReturn')}}:</span>
          <span>{{ getExpectedReturn(item) | moneyFormat  }}</span>
        </div>
...
name: "coinorder",
  ...
  methods:{
    ...
    getExpectedReturn(item ){
      return item.exp_amount * item.interest_rate * item.hold_cycle + item.exp_amount
    },
    //判断能否提前取出
    canTakeout(item){
